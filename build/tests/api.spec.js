...
const worker = (0, _2.default)({ name: WorkerName, listen /*, level: LEVEL_OFF*/ }).add({ cmd: 'cmd1' }, request => request.act({ cmd: 'ping' })).add({ cmd: 'cmd2' }, request => request.act({ cmd: 'cmd1' }));
const client = (0, _2.default)({ name: 'client' /*, level: LEVEL_OFF */ }).api(WorkerName, listen);
...
describe('api', function () {
  ...
  it('#exec client from worker cmd2', () => client.act({ api: 'worker', cmd: 'cmd2' }).then(result => should.equal(result, 'pong')));
});
